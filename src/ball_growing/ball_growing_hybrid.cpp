...
void inline bg_top_down_step(Graph &g, vertex_set *frontier, vertex_set *next_frontier, 
    ...
    int &num_unvisited, int &num_frontier_edges, int &num_edges_checked, int iter) {
...
void ball_decomp_hybrid(Graph g, double beta, std::vector<std::unordered_set<int>> &collection, std::vector<int> &radii) {
    double alpha = 10.0;
    double gamma = 10.0;
    ...
    while (num_unvisited > 0) {
        if (last_step == TOP_DOWN) {
            ...
            if (should_switch) {
                ...
                bg_top_down_step(g, frontier, next_frontier, unvisited, deltas, ball_ids, owner, num_unvisited, num_frontier_edges, num_edges_checked, iter);
