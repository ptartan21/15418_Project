...
void grow_ball(Graph g, int source, float beta, std::vector<bool> &present, std::unordered_set<int> &ball, int &R) {
    ...
    int degree_sum = 0;
    for (int eid = g->out_offsets[source]; eid < g->out_offsets[source+1]; ++eid) {
        int nid = g->out_edge_list[eid];
        if (present[nid] == PRESENT) {
            degree_sum++;
        }
    }
    ...
    while (isoperimetric_num > beta) {
        ...
        for (const auto &vid : boundary_vertices) {
            ...
            for (int eid = g->out_offsets[vid]; eid < g->out_offsets[vid+1]; ++eid) {
                int nid = g->out_edge_list[eid]; // this could be in boundary_vertices
                ...
                if (next_boundary_vertices.find(nid) == next_boundary_vertices.end() && boundary_vertices.find(nid) == boundary_vertices.end()) {
                    next_boundary_vertices.insert(nid);
                    for (int neid = g->out_offsets[nid]; neid < g->out_offsets[nid+1]; ++neid) {
                        int nnid = g->out_edge_list[neid];
                        if (present[nnid] == PRESENT) {
                            degree_sum++;
                            boundary_size++;
                        }
                    }
                ...
                // If edge goes from boundary -> next_boundary, decrement boundary size
                // If edge goes from boundary
            }
        }
        ...
        int boundary_size = 0;
        // Brute force compute boundary size
        for (auto &vid : ball) {
            for (int eid = g->out_offsets[vid]; eid < g->out_offsets[vid+1]; ++eid) {
                int nid = g->out_edge_list[eid];
                if (present[nid] == PRESENT) {
