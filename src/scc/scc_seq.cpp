...
void forward_reachability(Graph &g, unsigned char *S, unsigned char *reach, int vi, int method) {
    if (method == 0) {
        bfs_bottom_up_seq(g, S, reach, vi);
    } else if (method == 1) {
        bfs_top_down_seq(g, S, reach, vi);
    }
}

// flip the edges in the graph
void backward_reachability(Graph &g, unsigned char *S, unsigned char *reach, int vi, int method) {
    if (method == 0) {
        bfs_bottom_up_seq(g, S, reach, vi);
    } else if (method == 1) {
        bfs_top_down_seq(g, S, reach, vi);
...
void compute_scc_seq(std::vector<unsigned char *> &all_scc, Graph &g, int method) {
    ...
    for (int vid = 0; vid < n; ++vid) {
        ...
        if (S) {
            ...
            forward_reachability(g, S, r_plus, vid, method);            // populate r_plus with all forward reachable vertices
            backward_reachability(rev_g, S, r_minus, vid, method);      // populate r_minus with all backward reachable vertices
