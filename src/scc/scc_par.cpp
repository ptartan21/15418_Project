// Parallel Implementation for Finding Strongly Connected Components
#include <chrono>
#include <vector>
...
void forward_reachability_par(Graph &g, unsigned char *S, unsigned char *reach, int vi, int method) {
    if (method == 0) {
        bfs_bottom_up_par(g, S, reach, vi);
    } else if (method == 1) {
        bfs_top_down_par(g, S, reach, vi);
    }
}

// flip the edges in the graph
void backward_reachability_par(Graph &g, unsigned char *S, unsigned char *reach, int vi, int method) {
    if (method == 0) {
        bfs_bottom_up_par(g, S, reach, vi);
    } else if (method == 1) {
        bfs_top_down_par(g, S, reach, vi);
...
void compute_scc_par(std::vector<unsigned char *> &all_scc, Graph &g, int method) {
    ...
    for (int vid = 0; vid < n; ++vid) {
        ...
        if (S) {
            ...
            forward_reachability_par(g, S, r_plus, vid, method);            // populate r_plus with all forward reachable vertices
            backward_reachability_par(rev_g, S, r_minus, vid, method);      // populate r_minus with all backward reachable vertices
