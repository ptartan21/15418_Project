...
std::unordered_set<int> forward_reachability_par(Graph &g, std::unordered_set<int> &S, int vi, int method) {
    std::unordered_set<int> fr_vertices;
    if (method == 0) {
        fr_vertices = bfs_bottom_up_par(g, S, vi);
    } else if (method == 1) {
        fr_vertices = bfs_top_down_par(g, S, vi);
    }
    return fr_vertices;
}

// flip the edges in the graph
std::unordered_set<int> backward_reachability_par(Graph &g, std::unordered_set<int> &S, int vi, int method) {
    std::unordered_set<int> bw_vertices;
    if (method == 0) {
        bw_vertices = bfs_bottom_up_par(g, S, vi);
    } else if (method == 1) {
        bw_vertices = bfs_top_down_par(g, S, vi);
    }
    return bw_vertices;
}
...
void compute_scc_par(std::vector<std::unordered_set<int>> &all_scc, Graph &g, int method) {
    ...
    for (int vid = 0; vid < g->n; ++vid) {
        ...
        for (auto &s : V) {
            if (s.count(vid)) {
                S = s;
                std::unordered_set<int> r_plus  = forward_reachability_par(g, S, vid, method);
                std::unordered_set<int> r_minus = backward_reachability_par(rev_g, S, vid, method);
